  ...
  async onSubmit() {
    ...
    try {
      if (this.isEditMode) {
        ...
        await this.categoryService.updateCategory(this.data.id, this.data);
      } else {
        // Llamar al método de creación
        await this.categoryService.createCategory(this.data);
      }
      ...
      console.error(`Error al ${this.isEditMode ? 'actualizar' : 'crear'} categoría:`, error);
